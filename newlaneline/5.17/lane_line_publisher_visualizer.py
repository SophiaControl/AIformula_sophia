...
DEFAULT_MASK_DIR = (
    r"C:\Users\17396\Desktop\newlaneline\5.17"
    r"\aiformula_perception__road_detector__mask_image"
)
...
class Config:
    ...
    init_top_crop_ratio: float = 0.52
    tolerance: int = 10
    ...
    max_abs_line_slope: float = 4.50
    global_roi_top_row_ratio: float = 0.50
    global_roi_step_row_ratio: float = 0.72
    global_roi_inner_left_ratio: float = 0.18
    global_roi_inner_right_ratio: float = 0.82
    global_roi_outer_left_ratio: float = 0.00
    global_roi_outer_right_ratio: float = 1.00
    row_jump_far: float = 28.0
    row_jump_near: float = 110.0
    ...
    def margin_far(self) -> float:
        return max(40.0, 1.2 * float(max(1, self.tolerance)))

    @property
    def margin_near(self) -> float:
        return max(120.0, 4.0 * float(max(1, self.tolerance)))
...
class LaneLinePublisherEquivalent:
    ...
    def find_lane_lines(self, mask: np.ndarray) -> FrameResult:
        ...
        filtered_mask = remove_small_components(binary_mask, self.cfg.min_area)
        cleaned_mask, global_roi_polygon = apply_global_image_roi_crop(filtered_mask, self.cfg)
        ...
        if both_sides_conflict(side_results, cleaned_mask.shape[0], self.cfg):
            for side in (LEFT_INDEX, RIGHT_INDEX):
                side_results[side].status = "rejected_lane_spacing"
                side_results[side].vehicle_points = empty_points3()
                side_results[side].fitted_points = empty_points3()
                side_output_valid[side] = False
                self.register_detection_failure(self.stored_lines[side], force_reset=True)
                side_results[side].lost_count = self.stored_lines[side].lost_count
                contour_points[side] = empty_points3()
    ...
    def _process_side(self, cleaned_mask: np.ndarray, side: int) -> SideResult:
        ...
        vehicle_points_before_crop = pixels_to_vehicle_points(extracted_pixels, self.cfg)
    ...
    def make_annotated_mask_image(self, cleaned_mask: np.ndarray, global_roi_polygon: np.ndarray) -> np.ndarray:
        ...
        draw_global_image_roi_border(annotated, global_roi_polygon)
        for side in (LEFT_INDEX, RIGHT_INDEX):
...
def row_jump_limit_at_row(row: np.ndarray | int, rows: int, cfg: Config) -> np.ndarray | float:
    v_top = top_row(rows, cfg)
    v_bottom = max(v_top + 1, rows - 1)
    ratio = (np.asarray(row, dtype=np.float64) - float(v_top)) / float(v_bottom - v_top)
    ratio = np.clip(ratio, 0.0, 1.0)
    return cfg.row_jump_far + (cfg.row_jump_near - cfg.row_jump_far) * ratio


def lane_spacing_at_row(row: np.ndarray | int, rows: int, cfg: Config) -> np.ndarray | float:
    v_top = top_row(rows, cfg)
    v_bottom = max(v_top + 1, rows - 1)
    ratio = (np.asarray(row, dtype=np.float64) - float(v_top)) / float(v_bottom - v_top)
    ratio = np.clip(ratio, 0.0, 1.0)
    return cfg.min_lane_spacing_far + (cfg.min_lane_spacing_near - cfg.min_lane_spacing_far) * ratio


def base_side_bounds_at_row(side: int, row: int, rows: int, cols: int, cfg: Config) -> Tuple[int, int]:
    half_gap = float(lane_spacing_at_row(row, rows, cfg)) * 0.5
...
def build_global_image_roi_polygon(rows: int, cols: int, cfg: Config) -> np.ndarray:
    v_top = clamp_int(round(rows * cfg.global_roi_top_row_ratio), 0, rows - 1)
    v_step = clamp_int(round(rows * cfg.global_roi_step_row_ratio), v_top, rows - 1)
    u_inner_left = clamp_int(round(cfg.global_roi_inner_left_ratio * (cols - 1)), 0, cols - 1)
    u_inner_right = clamp_int(round(cfg.global_roi_inner_right_ratio * (cols - 1)), 0, cols - 1)
    u_outer_left = clamp_int(round(cfg.global_roi_outer_left_ratio * (cols - 1)), 0, cols - 1)
    u_outer_right = clamp_int(round(cfg.global_roi_outer_right_ratio * (cols - 1)), 0, cols - 1)
    return np.array(
        [
            [u_outer_left, rows - 1],
            [u_outer_right, rows - 1],
            [u_outer_right, v_step],
            [u_inner_right, v_top],
            [u_inner_left, v_top],
            [u_outer_left, v_step],
        ],
...
def apply_global_image_roi_crop(cleaned_mask: np.ndarray, cfg: Config) -> Tuple[np.ndarray, np.ndarray]:
    ...
    polygon = build_global_image_roi_polygon(rows, cols, cfg)
    roi_mask = np.zeros((rows, cols), dtype=np.uint8)
    if len(polygon) >= 3:
        cv2.fillPoly(roi_mask, [polygon.astype(np.int32)], 255)
    cropped = cv2.bitwise_and(cleaned_mask, roi_mask)
...
def extract_init_pixels(
    ...
    roi_mask = build_init_roi_mask(cleaned_mask.shape, side, opposite_line, cfg)
    return select_best_component(cleaned_mask, roi_mask, side, None)
...
def extract_tracking_pixels(
    ...
    roi_mask = build_tracking_roi_mask(cleaned_mask.shape, previous_line, side, opposite_line, cfg)
    return select_best_component(cleaned_mask, roi_mask, side, previous_line)


def build_init_roi_mask(
    ...
    for row in range(v_top, rows):
        ...
        if opposite_line.valid:
            gap = float(lane_spacing_at_row(row, rows, cfg))
...
def build_tracking_roi_mask(
    ...
    for row in range(v_top, rows):
        ...
        margin = float(margin_at_row(row, rows, cfg))
...
def component_pixels(binary_mask: np.ndarray) -> List[np.ndarray]:
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(binary_mask, 8, cv2.CV_32S)
...
def score_component_for_init(pixels: np.ndarray, side: int, center_u: float) -> float:
    if side == LEFT_INDEX:
        distances = center_u - pixels[:, 0].astype(np.float64)
    else:
        distances = pixels[:, 0].astype(np.float64) - center_u
    valid = distances >= 0.0
    if not np.any(valid):
        return float("inf")
...
def score_component_for_tracking(pixels: np.ndarray, line: ImageLineFit) -> float:
    pred = line.a * pixels[:, 1].astype(np.float64) + line.b
    return float(np.mean(np.abs(pixels[:, 0].astype(np.float64) - pred)))
...
def select_best_component(
    ...
    components = component_pixels(masked)
    if not components:
    ...
    for pixels in components:
        if reference_line is not None and reference_line.valid:
            score = score_component_for_tracking(pixels, reference_line)
        else:
            score = score_component_for_init(pixels, side, center_u)
        if score < best_score:
            best_score = score
            best_pixels = pixels
    return best_pixels
...
def has_insufficient_lane_spacing(
    ...
    v_top = top_row(rows, cfg)
    v_bottom = rows - 1
    current_top = current_line.a * float(v_top) + current_line.b
    current_bottom = current_line.a * float(v_bottom) + current_line.b
    opposite_top = opposite_line.a * float(v_top) + opposite_line.b
    opposite_bottom = opposite_line.a * float(v_bottom) + opposite_line.b

    spacing_top = abs(opposite_top - current_top)
    spacing_bottom = abs(opposite_bottom - current_bottom)
    return (
        spacing_top < float(lane_spacing_at_row(v_top, rows, cfg))
        or spacing_bottom < float(lane_spacing_at_row(v_bottom, rows, cfg))
    )


def both_sides_conflict(side_results: Dict[int, SideResult], rows: int, cfg: Config) -> bool:
    left = side_results[LEFT_INDEX]
    right = side_results[RIGHT_INDEX]
    if left.fitted_points.size == 0 or right.fitted_points.size == 0:
        return False
    return has_insufficient_lane_spacing(left.current_line, right.current_line, rows, cfg)
...
def pixels_to_vehicle_points(pixels: np.ndarray, cfg: Config) -> np.ndarray:
    ...
    dir_vehicle = dir_cam @ cfg.vehicle_R_camera
    dz = dir_vehicle[:, 2]
...
def draw_global_image_roi_border(image: np.ndarray, polygon: np.ndarray) -> None:
    if len(polygon) >= 3:
        overlay = image.copy()
        cv2.fillPoly(overlay, [polygon.astype(np.int32)], (80, 60, 0))
        cv2.addWeighted(overlay, 0.22, image, 0.78, 0.0, image)
        cv2.polylines(image, [polygon.astype(np.int32)], True, (255, 255, 0), 2)


def build_init_roi_polygon(image_shape: Tuple[int, int], side: int, cfg: Config) -> np.ndarray:
    rows, cols = image_shape[:2]
    v_top = top_row(rows, cfg)
    left_boundary: List[Tuple[int, int]] = []
    right_boundary: List[Tuple[int, int]] = []
    for v in range(v_top, rows):
        u_min, u_max = base_side_bounds_at_row(side, v, rows, cols, cfg)
        left_boundary.append((u_min, v))
        right_boundary.append((u_max, v))
    polygon = left_boundary + list(reversed(right_boundary))
    return np.array(polygon, dtype=np.int32) if polygon else np.empty((0, 2), dtype=np.int32)


def build_tracking_roi_polygon(image_shape: Tuple[int, int], line: ImageLineFit, side: int, cfg: Config) -> np.ndarray:
    rows, cols = image_shape[:2]
    v_top = top_row(rows, cfg)
    left_boundary: List[Tuple[int, int]] = []
    right_boundary: List[Tuple[int, int]] = []
    for v in range(v_top, rows):
        ...
        base_min, base_max = base_side_bounds_at_row(side, v, rows, cols, cfg)
        u_min = max(base_min, clamp_int(math.floor(u_pred - margin), 0, cols - 1))
        u_max = min(base_max, clamp_int(math.ceil(u_pred + margin), 0, cols - 1))
        if u_min > u_max:
            continue
        left_boundary.append((u_min, v))
        right_boundary.append((u_max, v))
...
def draw_roi_polygon(image: np.ndarray, polygon: np.ndarray, side: int) -> None:
    if len(polygon) < 3:
        return
    color = side_color(side)
    overlay = image.copy()
    cv2.fillPoly(overlay, [polygon.astype(np.int32)], blend_color(color, (0, 0, 0), 0.35))
...
def draw_init_roi_border(image: np.ndarray, side: int, cfg: Config) -> None:
    draw_roi_polygon(image, build_init_roi_polygon(image.shape, side, cfg), side)


def draw_tracking_roi_border(image: np.ndarray, line: ImageLineFit, side: int, cfg: Config) -> None:
    draw_roi_polygon(image, build_tracking_roi_polygon(image.shape, line, side, cfg), side)


def draw_sparse_pixels(image: np.ndarray, pixels: np.ndarray, side: int, radius: int = 0) -> None:
    if len(pixels) == 0:
        return
    color = side_color(side)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        default=r"C:\Users\17396\Desktop\newlaneline\5.17",
        help="Output directory. Relative paths are kept inside the current working directory.",
    )
    parser.add_argument(
        "--profile",
...
def vehicle_range_text(points: np.ndarray, axis: int) -> Tuple[str, str]:
    if len(points) == 0:
        return "", ""
    values = points[:, axis]
    ...
    if len(finite) == 0:
        return "", ""
    return f"{float(finite.min()):.6f}", f"{float(finite.max()):.6f}"


def write_summary_rows(writer: csv.writer, frame_index: int, file_name: str, result: FrameResult) -> None:
    for side in (LEFT_INDEX, RIGHT_INDEX):
        side_result = result.side_results[side]
        x_min, x_max = vehicle_range_text(side_result.vehicle_points_before_crop, 0)
        y_min, y_max = vehicle_range_text(side_result.vehicle_points_before_crop, 1)
        writer.writerow(
            [
                frame_index,
                file_name,
